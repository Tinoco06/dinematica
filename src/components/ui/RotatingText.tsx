/**
 * RotatingText — adaptado de ReactBits (DavidHDev/react-bits)
 * Cicla entre textos con animación de entrada/salida por caracteres o palabras.
 * Usa motion/react (ya en el stack).
 */
import {
  forwardRef,
  useCallback,
...
export const RotatingText = forwardRef<RotatingTextHandle, RotatingTextProps>(
  (props, ref) => {
    ...
    return (
      <motion.span
        className={`rotating-text ${mainClassName ?? ''}`}
        {...rest}
        layout
        transition={transition}
      >
        ...
        >
          <motion.span
            ...
            className={
              splitBy === 'lines' ? 'rotating-text-lines' : 'rotating-text'
            }
            layout
            aria-hidden="true"
          >
            {elements.map((wordObj, wordIndex, array) => {
              ...
              return (
                ...
                >
                  ...
                  {wordObj.needsSpace && (
                    <span className="rotating-text-space"> </span>
                  )}
                </span>
              )
            })}
